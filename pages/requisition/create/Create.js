...
//TODO Constructor
    ...
    async purchaseType(){
        try {
            let purchaseType = TestData.orderDetails.purchaseType;
            let prTypeLocator = await getPrType(purchaseType);
        ...
        try {
            ...
            switch (purchaseType.toLowerCase()) {
                case "catalog":
                    ...
                    await catalogTitleLocator.fill(title + "-" + purchaseType);
                    break;
                case "non-catalog":
                    ...
                    await nonCatalogTitleLocator.fill(title + "-" + purchaseType);
                    break;
                default:
    ...
    async project() {
        try {
            ...
            projectSearchFieldLocator.fill(projectCode);

            const dropDownValues = await this.page.locator('.select2-results__options');
            const dropDownOptions = await dropDownValues.allTextContents();
            for(let val of dropDownOptions){
                if(val.includes(projectCode)){
